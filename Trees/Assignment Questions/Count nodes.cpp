...
// Given a tree an integer x find and return the number of nodes which contains data greater than x.
...
int getLargeNodeCount(TreeNode<int> *root, int x)
{
    ...
    {
        greaterNodes += getLargeNodeCount(root->children[i], x);
    }
    return (root->data > x) ? (greaterNodes + 1) : greaterNodes;
}
